...
class Post_Purchase : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnAddPost.setOnClickListener {
            ...
            val category = "공동구매" // 카테고리는 일단 하드코딩으로 설정
        ...
        apiService.write(postsRequestDto).enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@Post_Purchase, "게시글이 등록되었습니다.", Toast.LENGTH_SHORT).show()
                    finish() // 액티비티 종료
                    val intent = Intent(this@Post_Purchase, Community_Purchase::class.java)
                    startActivity(intent)
                } else {
                    // 게시글 등록 실패 시 사용자에게 알림
                    Toast.makeText(this@Post_Purchase, "게시글 등록에 실패했습니다.", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                // 네트워크 오류 등의 이유로 API 호출 실패 시 사용자에게 알림
                Toast.makeText(this@Post_Purchase, "네트워크 오류: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}